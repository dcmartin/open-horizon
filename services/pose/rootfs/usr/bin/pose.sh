...
## initialize servive
CONFIG=$(echo $(pose_init) | jq '.resolution="'${WEBCAM_RESOLUTION}'"|.device="'${WEBCAM_DEVICE}'"')
...
hzn::log.notice "processing images from /dev/video0 every ${POSE_PERIOD} seconds"
...
while true; do
  ...
  POSE_OUTPUT_FILE=$(pose_process "${JPEG_FILE}" "${ITERATION}")

  # remove
  rm -f ${JPEG_FILE}

  if [ -s "${POSE_OUTPUT_FILE}" ]; then
    # add two files
    jq '.timestamp="'$(date -u +%FT%TZ)'"|.date='$(date +%s) "${POSE_OUTPUT_FILE}" > "${OUTPUT_FILE}"
    # update
  else
  ...
  # wait for ..
  SECONDS=$((POSE_PERIOD - $(($(date +%s) - DATE))))
