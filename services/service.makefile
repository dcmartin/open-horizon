...
GPU = $(if $(CUDA),$(shell echo "$(BUILD_ARCH)" | sed 's/[^_]*_\([^_]*\).*/\1/'),)
...
build: Dockerfile build.json $(SERVICE_JSON) makefile ${SERVICE_OPTIONS}
	...
	@export DOCKER_TAG="${DOCKER_TAG}" && docker build --progress=tty --build-arg GPU=$(if $(GPU),1,0) --build-arg BUILD_REF=$$(git rev-parse --short HEAD) --build-arg BUILD_DATE=$$(date -u +"%Y-%m-%dT%H:%M:%SZ") --build-arg BUILD_ARCH="$(BUILD_ARCH)" --build-arg BUILD_FROM="$(BUILD_FROM)" --build-arg BUILD_VERSION="${SERVICE_VERSION}" . -t "$(DOCKER_TAG)" | tee ${BUILD_OUT}
