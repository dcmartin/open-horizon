...
curl -sL -u "${HZNMONITOR_EXCHANGE_ORG}/${HZNMONITOR_EXCHANGE_USER:-iamapikey}:${HZNMONITOR_EXCHANGE_APIKEY}" "${HZNMONITOR_EXCHANGE_URL%/}/orgs/${HZNMONITOR_EXCHANGE_ORG}/users" \
  | jq '{"exchange":"'${HZNMONITOR_EXCHANGE_URL}'","org":"'${HZNMONITOR_EXCHANGE_ORG}'","user":"'${HZNMONITOR_EXCHANGE_USER}'","users":[.users|to_entries[]|.value.id=.key|.value]}'
