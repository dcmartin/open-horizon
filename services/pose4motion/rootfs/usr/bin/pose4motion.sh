...
if [ -z "${POSE4MOTION_CAMERA:-}" ]; then POSE4MOTION_CAMERA='+'; fi
if [ -z "${POSE4MOTION_TOPIC_EVENT:-}" ]; then POSE4MOTION_TOPIC_EVENT='event/end'; fi
if [ -z "${POSE4MOTION_TOPIC_PAYLOAD:-}" ]; then POSE4MOTION_TOPIC_PAYLOAD='image'; fi
if [ -z "${POSE4MOTION_TOO_OLD:-}" ]; then POSE4MOTION_TOO_OLD=300; fi

## derived
POSE4MOTION_TOPIC="${MOTION_GROUP}/${MOTION_CLIENT}/${POSE4MOTION_CAMERA}"

###
### FUNCTIONS
###

source /usr/bin/pose-tools.sh
source /usr/bin/service-tools.sh

## process JPEG image through POSE
process_pose()
{
  hzn::log.debug "${FUNCNAME[0]} ${*}"

  local input_jpeg_file=${1}
  local pose_json_file=$(pose_process "${input_jpeg_file}")
...
process_motion_event()
{
  ...
  # process image
  pose_json_file=$(process_pose "${input_jpeg_file}")

  if [ -s "${pose_json_file}" ]; then

    # extract image
    local output_jpeg=$(mktemp)
    jq -r '.image' "${pose_json_file}" | base64 --decode > ${output_jpeg}
    if [ -s "${output_jpeg}" ]; then
      local topic="${MOTION_GROUP}/${device}/${camera}/${POSE4MOTION_TOPIC_PAYLOAD%%/*}/pose/${POSE_PATTERN:-all}"
    ...
    # combine POSE output with service configuration file
    jq -s add "${service_json_file}" "${pose_json_file}" > "${service_json_file}.$$" && mv -f "${service_json_file}.$$" "${service_json_file}"
    # test for success
    if [ -s "${service_json_file}" ]; then
      local topic="${MOTION_GROUP}/${device}/${camera}/${POSE4MOTION_TOPIC_EVENT%%/*}/pose/${POSE_PATTERN:-all}"
      ...
      hzn::log.error "Failed to add JSON: ${service_json_file} and ${pose_json_file}"
    fi
  ...
  # cleanup
  rm -f ${payload} ${input_jpeg_file} ${pose_json_file} ${service_json_file}
}
...
CONFIG='{"timestamp":"'$(date -u +%FT%TZ)'","log_level":"'${LOG_LEVEL:-}'","debug":'${DEBUG:-false}',"group":"'${MOTION_GROUP:-}'","client":"'${MOTION_CLIENT}'","camera":"'${POSE4MOTION_CAMERA}'","event":"'${POSE4MOTION_TOPIC_EVENT:-}'","old":'${POSE4MOTION_TOO_OLD:-300}',"payload":"'${POSE4MOTION_TOPIC_PAYLOAD}'","topic":"'${POSE4MOTION_TOPIC}'","services":'"${SERVICES:-null}"',"mqtt":'"${MQTT}"',"pose":'$(pose_init)'}'

## initialize servive
service_init ${CONFIG}

# configure POSE
pose_config ${POSE_COUNTRY:-}
...
hzn::log.notice "Listening to MQTT host: ${MQTT_HOST}; topic: ${POSE4MOTION_TOPIC}/${POSE4MOTION_TOPIC_EVENT}"
...
## listen forever
mosquitto_sub ${MOSQUITTO_ARGS} -t "${POSE4MOTION_TOPIC}/${POSE4MOTION_TOPIC_EVENT}" | while read; do
  ...
  # test if too old
  if [ ${AGO} -gt ${POSE4MOTION_TOO_OLD} ]; then 
    hzn::log.warn "Too old; ${AGO} > ${POSE4MOTION_TOO_OLD}; continuing; PAYLOAD:" $(jq -c '.event.image=="redacted"' "${PAYLOAD}")
    continue
  fi
