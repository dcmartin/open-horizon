...
if [ ! -z "${1}" ]; then
  ID="${1}"
  curl -sL -u "${HZN_ORG_ID}/${HZN_USER_ID:-iamapikey}:${HZN_EXCHANGE_APIKEY}" "${HZN_EXCHANGE_URL%/}/orgs/${HZN_ORG_ID}/nodes/${ID}/status" \
    | jq '.={"status":.}' | jq '.name="'${ID}'"|.org="'${HZN_ORG_ID}'"|.url="'${HZN_EXCHANGE_URL}'"'
fi
